"""
Step 3: Hyperparameter search for SVM classifier.

Methodology follows Charlotte Model's Bachelor's thesis:
  - Search on a stratified 20% subset of the training data (not the full set)
  - 5-fold stratified cross-validation
  - Macro-F1 scoring
  - C range 0.01–4.0 (log-uniform), penalty l1 + squared_hinge (best in thesis)
  - Char n-grams (1,3)–(1,6), word n-grams (1,1)/(1,2), min_df 1/2

Best params written to models/svm_best_params.json.
"""
...
def main():
    ...
    start_run("step3_optimize_svm", artifacts=[PARAMS_PATH])
    ...
    for i, params in enumerate(param_samples, 1):
        ...
        print(f"[{i:2d}/{N_ITER}] char={char_range} word={word_range} "
              f"char_feats={char_feats} min_df_c={params['features__char__min_df']} "
              f"min_df_w={params['features__word__min_df']} C={c_val:.4f}")
    ...
    # Serialize: convert tuple ngram_range to list for JSON
    best = {
